...
def homework_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step2 = prover.add_instantiated_assumption('((Homework(x)&Fun(x))->Ex[(Homework(x)&Fun(x))])',
                                               prover.EI, {'R': '(Homework(_)&Fun(_))', 'x': 'x', 'c': 'x'})
    ...
    step7 = prover.add_tautological_implication('((Homework(x)&Reading(x))->(Reading(x)&~Fun(x)))', {step4, step6})
    step8 = prover.add_tautological_implication('((Homework(x)&Reading(x))->Ex[(Reading(x)&~Fun(x))])', {step5, step7})
    step9 = prover.add_assumption('Ex[(Homework(x)&Reading(x))]')
    prover.add_existential_derivation('Ex[(Reading(x)&~Fun(x))]', step9, step8)
...
def multiply_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step2 = prover.add_free_instantiation("plus(0,times(x,0))=times(x,0)", step1, {"x" : "times(x,0)"})
    step3 = prover.add_assumption("plus(minus(x),x)=0")
    step4 = prover.add_free_instantiation("plus(minus(times(x,0)),times(x,0))=0", step3, {"x" : "times(x,0)"})
    step5 = prover.add_flipped_equality("0=plus(minus(times(x,0)),times(x,0))", step4)
    step6 = prover.add_substituted_equality("plus(0,times(x,0))=plus(plus(minus(times(x,0)),times(x,0)),times(x,0))", step5, "plus(_,times(x,0))")
    ...
    step8 = prover.add_free_instantiation(
        ...
        step7, {'x': 'minus(times(x,0))', 'y': 'times(x,0)', 'z': 'times(x,0)'})

    step9 = prover.add_assumption('times(x,plus(y,z))=plus(times(x,y),times(x,z))')
    step10 = prover.add_free_instantiation('times(x,plus(0,0))=plus(times(x,0),times(x,0))', step9,
                                           {'x': 'x', 'y': '0', 'z': '0'})
    step11 = prover.add_flipped_equality('plus(times(x,0),times(x,0))=times(x,plus(0,0))', step10)
    step12 = prover.add_substituted_equality('plus(minus(times(x,0)),plus(times(x,0),'
                                             'times(x,0)))=plus(minus(times(x,0)),times(x,plus(0,0)))', step11,
                                             "plus(minus(times(x,0)),_)")
    step13 = prover.add_free_instantiation('plus(0,0)=0', step1, {'x': '0'})
    step14 = prover.add_substituted_equality(
        'plus(minus(times(x,0)),times(x,plus(0,0)))=plus(minus(times(x,0)),times(x,0))', step13,
        "plus(minus(times(x,0)),times(x,_))")
    step15 = prover.add_free_instantiation("plus(minus(times(x,0)),times(x,0))=0", step3, {'x': 'times(x,0)'})
    step16 = prover.add_assumption('times(x,y)=times(y,x)')
    step17 = prover.add_free_instantiation('times(0,x)=times(x,0)',
                                          step16, {'x': '0', 'y': 'x'})
    step18 = prover.add_flipped_equality("times(x,0)=plus(0,times(x,0))",
                                        step2)
    prover.add_chained_equality("times(0,x)=0", [step17, step18, step6, step8, step12, step14, step15])
...
def peano_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step2 = prover.add_assumption("plus(x,s(y))=s(plus(x,y))")

    step3 = prover.add_free_instantiation('plus(0,s(x))=s(plus(0,x))', step2, {'x': '0', 'y': 'x'})
    step4 = prover.add_free_instantiation("plus(0,0)=0", step1, {"x": "0"})

    ass = "(plus(0,x)=x->(plus(0,s(x))=s(plus(0,x))->plus(0,s(x))=s(x)))"
    step5 = prover.add_instantiated_assumption(ass, prover.ME, {"c":"plus(0,x)", "d":"x", "R":"plus(0,s(x))=s(_)"})
    ...
    step8 = prover.add_tautological_implication('(plus(0,0)=0&Ax[(plus(0,x)=x->plus(0,s(x))=s(x))])', [step4, step7])

    form = "((plus(0,0)=0&Ax[(plus(0,x)=x->plus(0,s(x))=s(x))])->Ax[plus(0,x)=x])"
    step9 = prover.add_instantiated_assumption( form, Schema(Formula.parse("((R(0)&Ax[(R(x)->R(s(x)))])->Ax[R(x)])"), {'R'}), {"R": "plus(0,_)=_"})
    step10 = prover.add_tautological_implication('Ax[plus(0,x)=x]', [step8, step9])
    prover.add_universal_instantiation('plus(0,x)=x', step10, 'x')
